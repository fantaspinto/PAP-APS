...
export default async function handler(
  ...
  try {
    ...
    if (!serverId) {
      return res.status(400).json({ error: "Falha no id do Servidor" });
    }
      
    if (!channelId) {
      return res.status(400).json({ error: "Falha no id do Canal" });
    }
          
    if (!content) {
      return res.status(400).json({ error: "Falha no contiudo" });
    }
